...
bDat$Liq <- t(autoCorrBL)
bDat$Par <- t(autoCorrBP)
...
nbDat$Liq <- t(autoCorrNBL)
nbDat$Par <- t(autoCorrNBP)

testDat <- rbind(bDat, nbDat)
...
firstTurn <- lapply(1:19, function(y) liquidTurns[[y]]$tppos[1])
